# ==============================================================================
#                 人才分析报告AI (Talent Report AI) v4.2
#                    (BI & Quant Edition)
# ==============================================================================
# 版本说明:
# - 【核心升级】生成多份深度量化分析表格，模仿专业BI报告。
# - 【AI角色转变】AI不再进行初级分析，而是扮演解读已生成表格的数据分析师角色。
# - 【格式优化】确保输出的Word文档干净、专业，无多余符号。
# - 【稳定可靠】保留所有之前的bug修复和定制化适配。
# ==============================================================================
...
def setup_environment():
    print("--- 启动准备工作 (v4.2 BI量化版) ---")
    os.makedirs('output', exist_ok=True)
    os.makedirs('resumes', exist_ok=True)
    print("  - 输出与简历目录已确认。")

# --- 1. 【升级】Word报告生成器 (支持多表格) ---
def generate_word_report(narratives, tables, filename):
    print("✍️ 正在组装Word(.docx)报告 (BI版)...")
    try:
        doc = Document()
        ...
        for chapter in narratives['chapters']:
            ...
            # 根据章节标题，插入对应的表格
            if chapter['title'] == '招聘漏斗与转化分析' and 'status_distribution' in tables:
                add_df_to_doc(tables['status_distribution'], title="表1: 招聘各阶段候选人数量分布")
            
            if chapter['title'] == '核心技能图谱量化分析' and 'skills_distribution' in tables:
                add_df_to_doc(tables['skills_distribution'], title="表2: 综合高频技能词统计")
            
            if chapter['title'] == '必备技能 vs. 高价技能评估' and 'must_have_vs_high_value' in tables:
...
def initialize():
    ...
    try:
        ...
        gemini_model = genai.GenerativeModel('models/gemini-2.5-flash'); print("✅ 初始化成功！"); return notion, gemini_model, config
    except Exception as e:
...
def process_data_and_generate_narratives(gemini_model, jd_text, df_notion, resumes_text):
    ...
    chapters_prompts = [
        {"title": "执行摘要", "prompt": f"作为AI人才战略顾问，请基于以下所有表格数据，撰写一份高度概括的执行摘要，点明我们人才池在招聘漏斗、核心技能储备方面的现状、优势与挑战。\n\n招聘漏斗数据:\n{tables.get('status_distribution', pd.DataFrame()).to_markdown()}\n\n技能分布数据:\n{tables.get('skills_distribution', pd.DataFrame()).to_markdown()}"},
        {"title": "人才市场宏观分析", "prompt": "作为行业分析师，请单纯基于你的知识库，分析当前AI大模型算法工程师市场的供需情况，并详细阐述“必备技能”（如Python, PyTorch）和“高价技能”（如分布式训练, RLHF, CUDA优化）之间的区别和价值。"},
        {"title": "招聘漏斗与转化分析", "prompt": f"请解读下面的“招聘各阶段候选人数量分布”表格。分析我们的招聘漏斗形态是否健康？在哪个环节可能存在瓶颈或流失率过高的问题？并提出初步的观察。\n\n{tables.get('status_distribution', pd.DataFrame()).to_markdown()}"},
        {"title": "核心技能图谱量化分析", "prompt": f"请解读下面的“综合高频技能词统计”表格。基于这些数据，我们当前人才池的整体技术画像是怎样的？哪些技术方向的人才储备相对充足，哪些存在明显短板？\n\n{tables.get('skills_distribution', pd.DataFrame()).to_markdown()}"},
        {"title": "必备技能 vs. 高价技能评估", "prompt": f"这张表格对比了我们定义的两类技能的覆盖情况。请深入分析，我们的候选人池在满足“必备技能”基础要求方面做得如何？在吸引具备“高价技能”的顶尖人才方面又表现如何？这揭示了我们招聘策略的哪些问题？\n\n{tables.get('must_have_vs_high_value', pd.DataFrame()).to_markdown()}"},
        {"title": "战略行动建议", "prompt": f"综合以上所有表格和你的解读，请为我们的招聘团队提出3-5条具体的、可立即执行的战略建议，以优化招聘漏斗、弥补技能短板，并更有效地吸引高价值人才。"}
    ]
